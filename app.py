...
def get_sheet_data():
    ...
    try:
        ...
        manual = []
    try:
        dividends = sh.worksheet("dividends").get_all_records()
    except:
        dividends = []
    return trades, config, manual, dividends
...
def parse_dividends(dividend_rows):
    """Return total dividend income (USD) and a list of dividend events."""
    total = 0.0
    events = []
    for row in dividend_rows:
        try:
            amount = float(row.get("div_income", 0) or 0)
        except (ValueError, TypeError):
            amount = 0.0
        total += amount
        events.append({
            "date":         row.get("date", ""),
            "asset_class":  row.get("asset_class", "C&CE"),
            "amount_usd":   round(amount, 2),
            "note":         row.get("Note", row.get("note", "")),
        })
    return round(total, 2), sorted(events, key=lambda x: x["date"], reverse=True)
...
def build_nav_curve(trades, dividend_total, fx_rates, cfg, benchmark_ticker):
    """Reconstruct daily NAV. Dividend income is treated as a lump-sum cash
    inflow already included in the starting cash pool (added to starting_capital
    for NAV purposes so the curve reflects total return)."""
    ...
    def get_hist_fx(col, live_fallback):
        if col in prices.columns:
            return prices[col]
        return None

    hist_gbpusd = get_hist_fx("GBPUSD=X", fx_rates.get("GBP", 1.0))
    hist_eurusd = get_hist_fx("EURUSD=X", fx_rates.get("EUR", 1.0))
    ...
    holdings = {}
    # Seed cash with starting capital + all dividend income received to date
    cash = starting + dividend_total
    nav_series = []
    bench_series = []
    bench_start = None
...
def portfolio():
    try:
        trades, config_rows, manual_rows, dividend_rows = get_sheet_data()
        ...
        # Dividends
        dividend_total, dividend_events = parse_dividends(dividend_rows)
        ...
        # Cash = uninvested capital + all dividend income received
        cash        = cfg["starting_capital"] - total_cost + dividend_total
        ...
        nav_series, bench_series = build_nav_curve(
            trades, dividend_total, fx_rates, cfg, cfg["benchmark"]
        )
        ...
        return jsonify({
            ...
            "total_pnl":         round(total_mv - total_cost + realised_total + dividend_total, 2),
