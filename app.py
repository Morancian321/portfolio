...
def build_positions(trades, fx_rates, manual_map):
    ...
    for ticker, events in ticker_trades.items():
        ...
        # Read currency_override once per ticker from the first event
        ...
        for e in events_sorted:
            ...
            if action == 'OPEN':
                ...
                # Re-read override from each OPEN event in case it differs
                ...
                cost_basis -= avg * qty
                qty_held -= qty
                # Apply override for closed trade currency
                ...
                closed_trades.append({
                    ...
                    'entry_price': round(avg, 4), 'exit_price': round(price, 4),
                    'realised_pnl_usd': round(realised_usd, 2),
                    'date': e.get('date'), 'yf_ticker': yf_ticker, 'asset_class': asset_class,
                })
                ...
                realised_usd = realised * fx
                closed_trades.append({
                    'ticker': ticker, 'name': name, 'qty': qty_held,
                    'entry_price': round(avg, 4), 'exit_price': round(price, 4),
                    'realised_pnl_usd': round(realised_usd, 2),
                    'date': e.get('date'), 'yf_ticker': yf_ticker, 'asset_class': asset_class,
                })
        ...
        if qty_held > 0:
            ...
            # Apply currency_override: if set, treat as USD (fx = 1.0), suppress pence divide
            currency = currency_override if currency_override else get_currency(yf_ticker)
            pence = is_pence(yf_ticker) if not currency_override else False
            fx = fx_rates.get(currency, 1.0)
            ...
            logging.warning(
                f'DEBUG ticker={ticker} qty={qty_held} avg_price={avg_price} live_price={lp} '
                f'pence={pence} fx={fx} mv_usd={mv_usd} cost_usd={cost_usd} currency={currency}'
            )

            open_positions.append({
                'ticker': ticker, 'name': name, 'asset_class': asset_class,
                'direction': direction, 'quantity': qty_held,
...
def build_nav_curve(trades, fx_rates, cfg, benchmark_ticker):
    ...
    for dt in date_range:
        ...
        for e in events_by_date.get(ds, []):
            ...
            # Apply currency_override for NAV curve cash flow calculations
            currency_override = e.get('currency_override', '').strip().upper()
        ...
        for tk, h in holdings.items():
            ...
            # Apply currency_override stored in holdings dict
